...
// when running go build, set these cgo environment variables:
//   CGO_CFLAGS=-I$DB2DIR/include
//   CGO_LDFLAGS=-L$DB2DIR/lib
...
// On some platforms SQLHANDLE is a SQLINTEGER and on others it is a void *.
// SQL_NULL_HANDLE is a #define, so we need to define an actual variable
// to compare against.
SQLHANDLE NIL_HANDLE = (SQLHANDLE) SQL_NULL_HANDLE;
...
func (d Driver) nilEnv() bool {
	return d.envHandle == C.SQLHENV(C.NIL_HANDLE)
}

func (d *Driver) clearEnv() {
	d.envHandle = C.SQLHENV(C.NIL_HANDLE)
}
...
func NewDriver() (driver.Driver, error) {
	...
	ret := C.SQLAllocHandle(
		C.SQL_HANDLE_ENV,
		C.NIL_HANDLE,
		(*C.SQLHANDLE)(&d.envHandle),
	)
	if !success(ret) {
...
func (c conn) nilDbc() bool {
	return c.dbc == C.SQLHDBC(C.NIL_HANDLE)
}

func (c *conn) clearDbc() {
	c.dbc = C.SQLHDBC(C.NIL_HANDLE)
}
...
func (r *rows) Next(dest []driver.Value) error {
	...
	for i, _ := range dest {
		...
		case C.SQL_REAL, C.SQL_FLOAT, C.SQL_DOUBLE, C.SQL_DECIMAL:
